...
export class PromiseQWebRTCReader {
  constructor(conf) {
    ...
    console.log("PromiseQWebRTCReader constructor called with config:", {
      url: this.cleanUrl,
  ...
  checkReadyState = () => {
    ...
    if (!this.streamReady && this.trackCount > 0 && connectionReady) {
      console.log("Stream is now READY! Triggering onReady event");
      ...
      if (this.conf.onReady) {
        console.log("Calling onReady with streams:", streams.length);
        try {
          this.conf.onReady({
  ...
  onTrack = (evt) => {
    console.log(
      "onTrack called with streams:",
      evt.streams ? evt.streams.length : 0
    );
    ...
    this.trackCount++;
    console.log("Track count increased to:", this.trackCount);
